...
class GithubProxy:
    ...
    def get_job_by_id(self, project, pipeline_id):
        url = "https://{}/repos/{}/actions/runs/{}".format(self.host, project, pipeline_id)
        try:
...
def request(
    method, url, params=None, payload=None, timeout=10, data=None, without_check_status=False
):
    params = params or {}
    try:
        response = requests.request(
            method, url, params=params, json=payload, data=data, timeout=timeout
        )
...
def set_output(output):
    ...
    os.system('echo "{}" >> $GITHUB_OUTPUT'.format(values))
